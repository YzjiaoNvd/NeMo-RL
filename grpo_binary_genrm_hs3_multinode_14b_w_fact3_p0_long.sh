...
NAME="grpo_binary_hs3_16K_step240_clip_max_0.28_${MODEL_NAME}_lr_${lr}_temp_${temp}_kl_${kl}_grpo_bs_${grpo_bs}_rollout_${rollouts_per_prompt}_num_prompts_${prompts_per_step}_${reward}_fact3_long${data_version}"

RESULTS_DIR="/lustre/fs1/portfolios/llmservice/projects/llmservice_modelalignment_sft/users/yizhuj/NeMo-RL/results/${NAME}"
mkdir -p $RESULTS_DIR
...
COMMAND="cd ${GPFS} && ulimit -c 0 && uv run examples/run_grpo_binary_genrm_w_fact3.py \
...
sbatch \
    ...
    --job-name=grpo_binary_genrm_hs3_${MODEL_NAME}_${lr}_${rollouts_per_prompt}_${reward}${data_version}_w_fact3_long \
