...
def run_two_stage_evaluation(vllm_generation, dataloader, output_file):
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader)):
        try:
            ...
            # Parse fact-checking responses
            parsed_factcheck = [
                parse_fact_checking_response(resp) for resp in factcheck_responses
            ]
            ...
            for i, metadata in enumerate(batch["extra_env_info"]):
                ...
                factcheck_result = parsed_factcheck[i]
            ...
            for idx, (factcheck_resp, scoring_resp, metadata) in enumerate(zip(
                parsed_factcheck, scoring_responses, batch["extra_env_info"]
            )):
                result = {
...
def main():
    ...
    try:
        # Run evaluation
        run_two_stage_evaluation(vllm_generation, dataloader, config["eval"]["output_file"])
    finally:
        # Cleanup
