...
from nemo_rl.environments.genrm_environment_w_error import (
    TwoStageErrorDetectionEnvironment, 
    TwoStageMetadata,
    format_error_detection_stage_prompt,
)
...
def two_stage_genrm_data_processor(
    ...
    factcheck_prompt = format_error_detection_stage_prompt(context, response1, response2)
    ...
    metadata: TwoStageMetadata = {
        ...
        "factcheck_stage_complete": False,  # Always start with fact-checking
...
def setup_two_stage_training(config, tokenizer, dataset, val_dataset):
    ...
    # Remove the TwoStageGenerationWrapper - let GRPO handle multi-turn naturally
    # DON'T wrap the generation interface anymore
...
def setup_two_stage_data(tokenizer, data_config, env_configs):
    ...
    two_stage_env = TwoStageErrorDetectionEnvironment.options(
        runtime_env={
            "py_executable": get_actor_python_env(
                "nemo_rl.environments.genrm_environment_w_error.TwoStageErrorDetectionEnvironment"
            ),
